...
function buildArqueoHTML(d) {
  ...
  const negocio = (typeof NEGOCIO !== 'undefined' && NEGOCIO.nombre) ? NEGOCIO.nombre : 'YERCO';
  const medios = c.ventasPorMedio || {};
...
function _nombreArchivoArqueo(ext) {
  ...
  return 'YERCO_' + tipo + '_caja' + String(c.numero || 0).padStart(4, '0') + '_' + (c.fecha || '') + '.' + ext;
}
...
function _exportarArqueoPDF() {
  ...
  doc.text((typeof NEGOCIO !== 'undefined' && NEGOCIO.nombre) ? NEGOCIO.nombre : 'YERCO', M, 8.5);
  doc.setFontSize(10);
